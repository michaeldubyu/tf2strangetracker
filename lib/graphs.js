            ...
            $(".graph_daily").bind("plotselected", function(event, ranges){
                $("#zoom_in").click(function() {
                    if (initial_range!=ranges) {
                        ...
                        //show from - to in MM/DD/YYYY - 24HH:MM:SS format
                        
                        var time_from = ranges.xaxis.from;
                        var time_to = ranges.xaxis.to;
                    }
    ...
    if ($(".graph_weekly").length){
        ...
        function onOutboundReceivedWeekly(series) {
            ...
            $(".graph_weekly").bind("plotselected", function(event, ranges){
                $("#zoom_in").click(function() {
                    if (initial_range!=ranges) weekly_plot = $.plot($(".graph_weekly"), finalData, $.extend(true, {}, options, { xaxis: { min: ranges.xaxis.from, max: ranges.xaxis.to } }));
                    initial_range = ranges;
                });
